...
class filters():
    ...
    Attributes:
        ...
        c0: Coordinates of bluff body center.
        D: Diameter or approximated diameter bluff body.
        geolen: Measurements of geometry.
    ...
    def waketransverse(self, xtoD, data_name = 'Velocity', scalar = False, 
        ...
        # The coordinate along the central line is y
        coordinate = np.array(detector)[:, 1]
        if scalar == True:
            quantity = np.array(FS)[:, 0]
        elif scalar == False:
            quantity = np.array(FS)[:, quanchoose]
    ...
    def drag_coe_Cp(self, data_name = 'Pressure'):
        ...
        for j in range(points.GetNumberOfPoints()):
            ...
            FS.append((float(p) - self.p0)/(0.5 * self.rho0 * self.U0**2))
...
class plotting():
    ...
    """
    def __init__(self, xdata, ydata, x_label = None, y_label = None, 
                 title = None, multi = False):
        self.xdata = xdata
        self.ydata = ydata
